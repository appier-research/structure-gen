    ...
    def wrapper(*args, **kwargs):
        ...
        while True:
            ...
            # Retry on specific errors
                ...
                # Sleep for the delay
                e_str = str(e)
                print(Fore.YELLOW + f"Error encountered ({e_str}). Retry ({num_retries}) after {delay} seconds..." + Style.RESET_ALL)
                if 'ged as potentially violating our usage policy. Please try again with a different prompt.' in e_str:
                    delay = 1
                time.sleep(delay)
                # Increment the delay
